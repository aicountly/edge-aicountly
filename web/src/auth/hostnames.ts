 ...
 * edge.aicountly.com and edge.gh.aicountly.com. This is the fallback for
 * hosts the pattern does not cover — localhost above all.
 */
export const PRODUCT_KEY = (import.meta.env.VITE_PRODUCT_KEY ?? 'edge').trim() || 'edge'
...
/**
 * `edge.aicountly.com` and `edge.gh.aicountly.com` both resolve to
 * `edge`, which is what the portal expects in authentication_jump.
 */
