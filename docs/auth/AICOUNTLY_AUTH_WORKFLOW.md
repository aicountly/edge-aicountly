# AICOUNTLY auth workflow (Edge)

How Edge signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `edge.aicountly.com` (or `edge.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/edge?returnUrl={origin}/auth/callback`.
...
| Edge host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `edge.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `edge.aicountly.com/api` |
| `edge.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `edge.gh.aicountly.com/api` |
...
`edge` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://edge.gh.aicountly.com/api/health
...
curl -i -X POST https://edge.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://edge.gh.aicountly.com/api/global/login   # expect 404
```
